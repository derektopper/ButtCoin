...
The RPM spec file provided here is for buttcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://buttcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[buttcoin git repository](https://github.com/buttcoin/buttcoin) but are not part of
...
Currently only `Source10` is used, for the example `buttcoin.conf` file.
...
in the buttcoin git repository. At present only an SVG version of the buttcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the buttcoin
...
needed when 0.12.1 is released, a proper fix is already in the buttcoin git
master branch.
...
idea to build buttcoin Core against a static version of that library compiled
...
The `buttcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `buttcoin-server` package. At this point, most RPM based
...
The `buttcoin-server` package should have SELinux support. How to properly do
...
buttcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/buttcoin/). He has been
packaging buttcoin for Fedora at least since 2012.
...
4. I renamed the `buttcoin` package that contains the Qt GUI to `buttcoin-core` as
that appears to be how the general population refers to it, in contrast to
`buttcoin-xt` or `buttcoin-classic`. I wanted to make sure the general population
