#!/bin/bash
# Copyright (c) 2013 The buttcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete buttcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no buttcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old buttcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected buttcoin 0.7 datadir."
    ;;
  3)
    echo "Detected buttcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected buttcoin 0.8 datadir."
    ;;
esac
