...
const char* BIP71_MIMETYPE_PAYMENT = "application/buttcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/buttcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/buttcoin-paymentrequest";
...
//
// OSX-specific way of handling buttcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in buttcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(buttcoin_IPC_PREFIX, Qt::CaseInsensitive)) // buttcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom buttcoin addresses are not supported
        ...
        // buttcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(buttcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
